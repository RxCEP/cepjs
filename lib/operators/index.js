...
switchMap: rSwitchMap, takeUntil: rTakeUntil} = require('../rlib').operators;
...
const checkStream = (stream) => R.is(EventStream, stream) ? stream._observable : stream;
/**
 * 
 * @param {Function} projectionFn 
 * @return {EventStream} a new event stream instance.
...
function mergeMap(projectionFn){
    return function(stream){
        return new EventStream(stream._observable.pipe(rMergeMap(R.compose(checkStream, projectionFn))));
    }
}
/**
 * 
...
function concatMap(projectionFn){
    return function(stream){
        return new EventStream(stream._observable.pipe(rConcatMap(R.compose(checkStream, projectionFn))));
    }
}
/**
 * 
...
function switchMap(projectionFn){
    return function(stream){
        return new EventStream(stream._observable.pipe(rSwitchMap(R.compose(checkStream, projectionFn))));
    }
}
/**
 * 
 * @param {EventStream} eventStream - some event stream instance.
 * @return {EventStream} a new event stream instance.
 */
function takeUntil(eventStream){
    return function(stream){
        return new EventStream(stream._observable.pipe(rTakeUntil(eventStream._observable)));
    }
}
...
function groupBy(attribute){
    return function(stream){
        return new EventStream(stream._observable.pipe(customGroupBy(attribute)));
    }
}
