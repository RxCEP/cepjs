...
const { predEvtTypeList, compareEvtType, deriveEvt, isWindow,
    evtTypeListLengthOne, filterEvtsByEvtTypes } = require('../../helperFuntions');
...
const applyAssertion = product => product.filter(set => assertion ? R.apply(assertion, set) : true);
...
const all = (eventTypeList, newEvtTypeId, assertion) => (source) =>{
    ...
    return source.pipe(filter(elems => evtTypeListLengthOne(eventTypeList)),
            filter(isWindow), //checks if it's a window
            map(separateEvtsByType(eventTypeList, evtTypeListCounted)),
            map(R.apply(xprodN(evtTypeListCounted.length))), //calculates the cartesian product
            ...
            map(applyAssertion), //applies the assertion if it exists
            mergeMap(from),
            map(derivation));
}
...
const any = (eventTypeList, newEvtTypeId) => (source) => {
    ...
    return source.pipe(filter(isWindow), //checks if it's a window
