const { filter: rFilter, tap: rTap, map: rMap, mergeMap: rMergeMap,
    debounceTime: rDebounceTime, throttleTime: rThrottleTime, delay: rDelay,
    skip: rSkip, take: rTake, concatMap: rConcatMap, switchMap: rSwitchMap,
    takeUntil: rTakeUntil, retry: rRetry, share: rShare, merge: rMerge } = require('rxjs/operators');
 ...
 * @param {number} time 
 */
const debounceTime = time => rDebounceTime(time);

/**
 * 
 * @param {number} time 
 */
const throttleTime = time => rThrottleTime(time);

/**
 * 
 * @param {number} time 
 */
const delay = time => rDelay(time);
