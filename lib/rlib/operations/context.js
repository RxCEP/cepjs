 ...
 * @return {EventStream} a new event stream instance.
 */
const slidingCountWindow = n => stream =>
        stream.pipe(bufferCount(n, 1),filter(x => x.length === n));
 ...
 * @return {EventStream} a new event stream instance.
 */
const tumblingTemporalWindow = timeDuration => bufferTime(timeDuration);
 ...
 * @return {EventStream} a new event stream instance.
 */
const hoppingTemporalWindow = (timeDuration, hopSize) => bufferTime(timeDuration, hopSize);
...
const groupBy = attribute => stream => 
            stream.pipe(map(R.groupWith(R.eqBy(getProp(attribute)))), mergeMap(from));
