...
function of(/*...values, adaptor*/){
    ...
    let newEvtOccurence = new EventType('of event', 'of operation', currDate, currDate);

    return new EventStream(arguments.length > 1 && R.is(Function, adaptor)
        ? libOf(...(R.dropLast(1, args))).pipe(map(adaptor), map(R.set(detcTimeLens, currDate)))
...
function from(source, adaptor){
    const currDate = new Date();
    let newEvtOccurence = new EventType('from event', 'from operation', currDate, currDate);
...
function fromEvent(target, eventName, adaptor) {
    const currDate = new Date();
    let newEvtOccurence = new EventType('fromEvent event', 'fromEvent operation', currDate, currDate);
    
    return new EventStream(R.is(Function, adaptor)
        ? libFromEvent(target, eventName).pipe(map(adaptor), map(R.set(detcTimeLens, currDate)))
